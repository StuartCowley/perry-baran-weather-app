...
describe("getUnits", () => {
  ...
  it("returns standard units when passed an invalid string", () => {
    const { tempUnits, speedUnits } = getUnits("fish");
    expect(tempUnits).toEqual("K");
    expect(speedUnits).toEqual("m/s");
  });

  it("returns standard units when passed a value that isn't a string", () => {
  ...
  it("returns correct units when passed a string with improper capitilization", () => {
    const { tempUnits, speedUnits } = getUnits("mEtRiC");
    expect(tempUnits).toEqual("°C");
    expect(speedUnits).toEqual("m/s");
  });
